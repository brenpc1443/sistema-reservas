terraform {
  ...
  backend "s3" {
    ...
    key            = "hotel-reservas/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
...
resource "aws_security_group" "jenkins" {
  ...
  ingress {
    ...
    to_port     = 8080
    protocol    = "tcp"
    cidr_blocks = var.jenkins_allowed_ips # Solo IPs específicas
  }
...
resource "aws_cloudwatch_log_group" "ecs_pagos" {
  name              = "/ecs/${var.project_name}/pagos"
  retention_in_days = 7

  tags = {
    Name = "${var.project_name}-pagos-logs"
  }
}
...
resource "aws_ecr_repository" "pagos" {
  ...
  tags = {
    Name = "${var.project_name}-pagos-ecr"
  }
}
...
resource "aws_lb_target_group" "pagos" {
  name        = "${var.project_name}-pagos-tg"
  port        = 3000
  protocol    = "HTTP"
  vpc_id      = aws_vpc.main.id
  target_type = "ip"
...
# ============================================
# ECS TASK DEFINITION - PAGOS
# ============================================
resource "aws_ecs_task_definition" "pagos" {
  family                   = "${var.project_name}-pagos"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = aws_cloudwatch_log_group.ecs_pagos.name
...
resource "aws_ecs_service" "pagos" {
  name            = "${var.project_name}-pagos-service"
...
resource "aws_instance" "jenkins" {
  ...
  user_data = templatefile("${path.module}/scripts/jenkins-userdata.sh", {
    region       = var.aws_region
    ecr_repo_url = aws_ecr_repository.pagos.repository_url
  })
